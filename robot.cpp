...
void completion(double vLeft, double vRight) {
	...
	double motorR = vRight -15;
	double motorL = vLeft -15;
	...
	for (int pos = 0; pos < fovArraySize; pos++){
		...
		if(fov[pos] == 1){
			...
			if (averagePos >= 0 && averagePos <= fovArraySize/2 - 2){ // else if there is white on the left change motor speed to enable us to go to it
				motorR = vRight + averagePos/vRight * vRight/10;
			}
				...
				motorL = vLeft + averagePos/vLeft * vLeft/10;
			} 
...
void challenge(double vLeft, double vRight) {
